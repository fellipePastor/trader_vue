    ...
    <v-card>
      <v-container fill-height>
        <v-text-field label="Quantidade" 
                      ...
                      :error="insuffcientFunds ||!Number.isInteger(quantity)"
                      />
            
        <v-btn class="green darken-3 white--text" 
                    @click="buyStock()"
                    :disabled="insuffcientFunds || quantity <= 0 || !Number.isInteger(quantity)"
                    >{{insuffcientFunds ? 'Insuficiente' : 'Comprar'}}</v-btn>
      </v-container>
    </v-card>
  </v-flex>
</template>
